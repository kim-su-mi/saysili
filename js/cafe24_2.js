...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.loadSVGBackground = function(svgUrl) {
        // SVG 파일을 텍스트로 가져오기
        fetch(svgUrl)
        ...
        fetch(currentSvgUrl)
            ...
            .then(svgContent => {
                ...
                if (gradient2) {
                    ...
                    for (let stop of stops) {
                        stop.setAttribute('stop-color', window.adjustColor(baseColor, -20));
                    }
                }
    ...
    // 사이즈 버튼 상태 업데이트 함수
    window.updateSizeButtonState = function(activeId) {
...
// Canvas 상태 저장 함수
window.saveCurrentCanvasState = function() {
...
// Canvas 상태 로드 함수
window.loadCanvasState = function() {
